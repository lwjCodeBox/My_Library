...
CString CDlgInit::GetModulePath()
{
	CString sPath;
...
CString CDlgInit::GetBitmapFileName(UINT nBitmapID)
{
	switch (nBitmapID)
	{
...
CString CDlgInit::GetIconFileName(UINT nIconID)
{
	switch (nIconID)
	{
	case IDX_ICON_SKIN:				return _T("skin\\skin.ico");
	case IDX_ICON_SKIN_INACTIVE:	return _T("skin\\skin_inactive.ico");
//	case IDX_ICON_OK:				return _T("skin\\ok.ico");
//	case IDX_ICON_CANCEL:			return _T("skin\\close.ico");
//	case IDX_ICON_HOME:				return _T("skin\\home.ico");
//	case IDX_ICON_FAVORITES:		return _T("skin\\favorites.ico");
//	case IDX_ICON_INTERNET:			return _T("skin\\internet.ico");
//	case IDX_ICON_SETTING:			return _T("skin\\setting.ico");
	}
	return _T("");
}

SIZE CDlgInit::GetBitmapSize(UINT nBitmapID)
{
	SIZE size;
...
HBITMAP CDlgInit::GetBitmap(UINT nBitmapID)
{
	if (nBitmapID >= IDX_BMP_MAX)
		return NULL;
...
HICON CDlgInit::GetIcon(UINT nIconID, UINT nType)
{
...
void CDlgInit::SetRGB(UINT nRGBID, COLORREF cf)
{
...
COLORREF CDlgInit::GetRGB(UINT nRGBID)
{
	if (nRGBID >= IDX_RGB_MAX)
		return RGB(0, 0, 0);
...
CFont *CDlgInit::GetFont(UINT nFontID)
{
...
CFont *CDlgInit::SetFont(UINT nFontID, CString sFaceName, UINT nFontSize, BOOL bBold/*=FALSE*/)
{
	if (nFontID >= IDX_FONT_MAX)
		return NULL;
...
CSize CDlgInit::FormatText(CString &sText, CDC *pDC, int nWidth)
{
...
COLORREF CDlgInit::Darker(COLORREF crBase, float fFactor)
{
...
COLORREF CDlgInit::Lighter(COLORREF crBase, float fFactor)
{
	ASSERT(fFactor > 1.0f);
